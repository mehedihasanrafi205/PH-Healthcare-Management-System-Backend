              ...
              if (!result) {
                return reject(
                  new AppError(
                    ...
                    "No result returned from the Cloudinary",
                  ),
                );
              }
...
const approveDoctor = async (
  ...
  const templateData = {
    ...
    rejectionReason:
      updatedDoctor.rejectionReason ||
      "Requirements did not match our system guidelines.",
    year: new Date().getFullYear(),
  };
...
const getAvailableDoctorByTodaysSchedule = async (query: IQuery) => {
  ...
  if (query.doctorId) {
    andConditions.push({ id: query.doctorId });
  }
